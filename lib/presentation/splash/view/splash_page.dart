...
class _SplashPageWidget extends StatelessWidget {
  const _SplashPageWidget({super.key});
    ...
    return BlocListener<SplashBloc, SplashState>(
      listener: (context, state) {
        if (state is SplashUnauthorized) {
          unawaited(Navigator.of(context).pushReplacementNamed(RouteName.login.route));
        } else if (state is SplashAuthorized) {
          unawaited(Navigator.of(context).pushReplacementNamed(RouteName.home.route));
        }
      },
